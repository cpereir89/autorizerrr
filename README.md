...
**Autorizerrr** is an open-source **automatic authorization and authentication enforcement detection** extension for **Burp Suite**.
...
Autorizerrr passively monitors traffic generated by a **high-privileged user**, automatically replays requests using **low-privileged** or **unauthenticated** contexts, and analyzes the server responses to determine whether access controls are properly enforced.

The extension is written in Jython by **Barak Tawily**, an application security researcher and practitioner.

![Autorizerrr Screenshot](Autorize.png)
...
Autorizerrr is especially effective during **authenticated testing**, where traditional crawling and scanning tools fall short.
...
4. Install Autorizerrr
   - **Recommended**: Install directly from the **BApp Store**
   - **Manual installation**:
     ```bash
     git clone https://github.com/cpereir89/autorizerrr.git
     ```
     - Burp → **Extender** → **Extensions** → **Add**
     - Select `Autorize.py`

5. Verify installation
   - A new **Autorizerrr** tab should appear in Burp
...
1. Open the **Autorizerrr** tab after installation.
2. Navigate to **Autorizerrr → Configuration**.
...
8. Observe results in the Autorizerrr table.
...
Autorizerrr assigns one of the following statuses to each tested endpoint:
...
Autorizerrr could not reliably determine enforcement and requires **manual fingerprint configuration**.
...
Add this string (or a regex) as a fingerprint in the enforcement detector. Autorizerrr will then automatically recognize authorization enforcement for similar responses.

---

## Interception Filters

Interception filters control **which requests are analyzed** by Autorizerrr, helping reduce noise and improve performance.
...
> Autorizerrr includes default filters to exclude static assets such as images and scripts.

Example configuration:

![Interception Filters](interception-filters.png)

---

## Multi-User Comparison & Viewer Visibility

Autorizerrr supports **multiple low-privileged users** simultaneously, allowing you to compare authorization behavior across different roles or sessions.
...
- Autorizerrr relies on **response comparison heuristics** and fingerprints—false positives or false negatives are possible.
